...
export default class LeaderboardScene extends Phaser.Scene {
  ...
  create() {
    ...
    this.make.text({
      x: this.width / 2,
      y: this.height / 2,
      text: 'You can challenge this score with others, top players can earn RCR tokens',
      style: {
        fontSize: '30px',
        fill: '#ffffff',
        fontFamily: 'Arcadia, monospace',
    ...
    this.playBtn = this.add.image(this.width / 2, this.height / 2 + 120, 'play').setInteractive({ useHandCursor: true }).setOrigin(0.5, 0.5)
      .on('pointerup', () => {
        this.playNotPressed();
        this.start();
      });
